...
const socket = new WebSocket('ws://seu-servidor-websocket'); // Substitua pela URL do seu servidor WebSocket
...
function formatarDetalhesPedido(pedido) {
    return `Pedido: ${pedido.nomePedido}, Valor: R$ ${pedido.valor}, Pagamento: ${pedido.formaPagamento}, Troco: ${pedido.troco}, Status: ${pedido.statusPedido}.`;
}
...
socket.onmessage = (event) => {
    const { action, pedido } = JSON.parse(event.data);
    
    if (action === 'update') {
        // Lógica para atualizar a lista com o pedido recebido
        adicionarPedido(pedido);
    } else if (action === 'remove') {
        // Lógica para remover o pedido da lista
        removerPedido(pedido.nomeCliente);
    }
};
